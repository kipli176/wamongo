...
const Pesan = require('../models/chat.model')
            ...
            m.messages.map(async (msg) => {
                ...
                if (
                    ...
                    return
                        ...
                        switch (messageType) {
                            case 'conversation':
                                var isine = msg.message.conversation            
                                var nomer = msg.key.remoteJid.split('@')[0]  
                                var me = msg.key.fromMe 
                                var jam = msg.messageTimestamp
                                break
                            case 'imageMessage':
                            case 'videoMessage':
                            case 'audioMessage':
                            ...
                            case 'documentMessage':
                                ...
                                if (!fs.existsSync('./downloaded_/')) {
                                    fs.mkdirSync('./downloaded_/');
                                }
                                let fileName = msg.key.remoteJid + '_' + msg.key.id + '_' + msg.message[messageType].mediaKeyTimestamp + '.' + ext
                                await writeFile('./downloaded_/' + fileName, buffer) 
                                var nomer = msg.key.remoteJid.split('@')[0]    
                                var isine = `${config.appUrl}/downloaded_/${fileName}`
                                var me = msg.key.fromMe
                                var jam = msg.messageTimestamp
                                break;
                            default:
                    ...
                    const savePesan = new Pesan({ key: this.key,no:nomer,fromMe:me,jam:jam,pesan:isine })
                    await savePesan.save() 
